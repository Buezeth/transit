// app/[lang]/(admin)/dashboard/shipments/page.tsx
...
export default async function ShipmentsPage({ params }: { params: Promise<{ lang: 'en'|'fr' }> }) {
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold text-gray-900">{dict.shipments.title}</h1>
        ...
        <div className="flex gap-2">
          <form action={createShipment}>
            ...
            <button className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 flex items-center gap-2 text-sm font-medium">
              <Plus className="w-4 h-4" /> {dict.shipments.newAir}
            </button>
          </form>
          <form action={createShipment}>
            ...
            <button className="bg-teal-600 text-white px-4 py-2 rounded-md hover:bg-teal-700 flex items-center gap-2 text-sm font-medium">
              <Plus className="w-4 h-4" /> {dict.shipments.newSea}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {shipments.map((shipment) => (
          <Link key={shipment.id} href={`/${lang}/dashboard/shipments/${shipment.id}`} className="block group">
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:border-blue-300 transition-all">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{dict.shipments.created} {shipment.createdAt.toLocaleDateString()}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between mt-4 text-sm">
                <span className="text-gray-500">{shipment._count.packages} {dict.shipments.packages}</span>
                <span className="flex items-center text-blue-600 font-medium group-hover:translate-x-1 transition-transform">
                  {dict.shipments.manage} <ArrowRight className="w-4 h-4 ml-1" />
                </span>
              </div>
            </div>
          </Link>
